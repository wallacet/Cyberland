...
public sealed class Scene2DMathTests
{
    [Fact]
    public void LocalMatrix_translation_without_rotation()
    {
        var m = Transform2DMath.LocalMatrix(new Vector2D<float>(10f, 20f), 0f, new Vector2D<float>(1f, 1f));
    ...
    public void Compose_multiplies_parent_and_local()
    {
        ...
        var w = Transform2DMath.Compose(parent, local);
    ...
    {
        var m = Transform2DMath.MatrixFromPositionRotationScale(new Vector2D<float>(3f, 4f), 0f, new Vector2D<float>(1f, 1f));
        Transform2DMath.DecomposeToPRS(m, out var pos, out var rad, out var scale);
    ...
    public void DecomposeToPRS_handles_near_zero_scale()
    {
        ...
        Transform2DMath.DecomposeToPRS(m, out _, out var rad, out var scale);
        Assert.Equal(0f, rad);
