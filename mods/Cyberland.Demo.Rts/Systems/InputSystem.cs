...
public sealed class InputSystem : ISingletonSystem, ISingletonEarlyUpdate
{
    ...
    public SystemQuerySpec QuerySpec => SystemQuerySpec.All<SessionState, ControlGroups>();

    private readonly GameHostServices _host;
    private EntityId[] _selectedScratch = [];
    private Vector2D<float> _dragStartScreen;
    private readonly bool[] _prevDigitDown = new bool[ControlGroups.GroupCount];
    ...
    {
        _ = sessionRow;
        _selectedScratch = new EntityId[Constants.UnitCount];
    }
    ...
    public void OnSingletonEarlyUpdate(in SingletonEntity sessionRow, float deltaSeconds)
    {
        ...
        ref var session = ref sessionRow.Get<SessionState>();
        ref var groups = ref sessionRow.Get<ControlGroups>();
        ...
        if (input.ConsumeMouseButtonReleased(MouseButton.Left) && session.BoxDragActive)
        {
            ...
            if (dragPx < Constants.BoxSelectMinDragScreenPx)
        ...
        if (input.ConsumeMouseButtonPressed(MouseButton.Right))
        {
            ...
            if (selectedCount > 0)
            {
                ...
                Formation.AssignGridTargets(anchor, _selectedScratch.AsSpan(0, selectedCount), world);
                for (var i = 0; i < selectedCount; i++)
                {
                    ref var state = ref world.Get<UnitState>(_selectedScratch[i]);
        ...
        IInputService input)
    {
        ...
        for (var g = 0; g < ControlGroups.GroupCount; g++)
        {
            ...
            if (session.LastGroupKeyIndex == g &&
                session.GroupKeyClock - session.LastGroupKeyTime <= Constants.GroupDoubleTapSeconds)
            {
        ...
        World world)
    {
        ...
        var clamped = CameraBounds.ClampCenterPosition(
            centroid,
    ...
    private static void RecordGroupKeyTap(ref SessionState session, byte groupIndex)
    {
    ...
    private static void ApplyClickSelect(World world, Vector2D<float> worldPoint, RtsSelectionMode mode)
    {
        ...
        foreach (var chunk in world.QueryChunks(SystemQuerySpec.All<UnitTag, Transform, Sprite>()))
        {
        ...
        ref var hitState = ref world.Get<UnitState>(hit.Value);
