...
{
    /// <summary>
    /// Writes each unit's <see cref="UnitState.MoveTargetWorld"/> to a stable row-major grid slot centered on
    ...
    public static void AssignGridTargets(Vector2D<float> anchor, ReadOnlySpan<EntityId> units, World world)
    {
        ...
        var spacing = Constants.FormationSpacing;
        ...
        for (var i = 0; i < count; i++)
        {
            ...
            ref var state = ref world.Get<UnitState>(sorted[i]);
