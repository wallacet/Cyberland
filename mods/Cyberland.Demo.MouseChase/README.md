...
2. **`Content/Scenes/demo_mousechase.json`** + **`Content/Ui/mousechase_hud.json`** — world layout and retained HUD (`uiPath` on **`ui-document-root`**).
3. **`Mod.UiBind.cs`** — resolves **`HudDocumentRefs`** from JSON element ids after spawn.
...
- **`Content/Ui/mousechase_hud.json`** — retained HUD tree.
