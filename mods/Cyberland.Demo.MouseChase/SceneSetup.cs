...
public static class SceneSetup
{
    ...
    public static async ValueTask<HudDocumentRefs> SetupSceneAsync(ModLoadContext context, CancellationToken cancellationToken = default)
    {
        ...
        // Opaque world sprites need baseline lighting in the deferred path (see mod README).
    ...
    private static void ApplyGlobalPost(World world)
    {
        ...
        world.GetOrAdd<GlobalPostProcessSource>(e) = new GlobalPostProcessSource
        {
            ...
            {
                // Bright white HUD on dark bg blooms heavily; blur reads like persistent text—disable until UI can mask bloom.
                BloomEnabled = false,
                BloomRadius = 1f,
                BloomGain = 0.18f,
