...
This repo includes a pre-commit hook that blocks staged files larger than 4 MiB by default, rejects commits when any **shipped demo mod** manifest (`mods/Cyberland.Demo*/manifest.json`) is not **`"disabled": true`** in the staging area (so local testing can enable demos without that state landing in git), and when you stage **`Cyberland.sln`** or anything under **`mods/`**, runs the same **mod projects in solution** check as the **`Mod Projects In Solution`** GitHub Action (`scripts/assert-mod-projects-in-solution.sh`, requires **`bash`** on `PATH`, as with Git for Windows).
...
- Skip mod/solution membership check (use sparingly): `CYBERLAND_SKIP_MOD_SLN_CHECK=1 git commit ...`
...
**Default engine systems (2D):** Two shipped engine mods register stock systems through **`EngineDefaultSchedulerSystems`**: **`cyberland.engine.early`** (load order **`-10000`**) registers parallel **transform hierarchy**, **sprite animation**, **`SpriteAtlasAnimationSystem`** (**`cyberland.engine/sprite-atlas-animation`**), and **CPU particle simulation** before gameplay mods; **`cyberland.engine.late`** (load order **`10000`**) registers post-gameplay systems in fixed order: parallel **`CameraFollowSystem`** (**`cyberland.engine/camera-follow`**) and **`TriggerSystem`** (**`cyberland.engine/trigger`**), serial camera submit/runtime/layout (**`cyberland.engine/camera-submit`**, **`.../camera-runtime-state`**, **`.../viewport-layout`**), parallel lighting/post-volume submitters, parallel **tilemap** submit, serial **`SpriteLocalizedAssetSystem`** (**`cyberland.engine/sprite-localized-assets`**) and **`SpriteAtlasBindingSystem`** (**`cyberland.engine/sprite-atlas-bindings`**), parallel **sprite** / **particle** / **text staging** / **text render**, and serial UI document systems. This split lets modpacks disable or replace stock engine systems with normal mod tools (`SetSystemEnabled`, `TryUnregister`, re-register by id). Keep gameplay mods in the **middle** load-order band so they run between early and late engine passes; excluding `cyberland.engine.early` or `cyberland.engine.late` with `--exclude-mods` removes that default behavior unless another modpack provides replacements. Prefer these ECS-driven paths over calling **`Submit*Light`** / **`SubmitPostProcessVolume`** / **`SubmitCamera`** from mod code unless you need a special-case. After **`VulkanRenderer`** initialization, **`GameApplication`** applies baseline HDR globals once via **`EngineDefaultGlobalPostProcess.Apply`** (not every frame). Mods replace or extend those settings by calling **`SetGlobalPostProcess`** from **`IMod.OnLoadAsync`** (later loads win over earlier ones) or from a system when values must track the frame. **`BitmapText`** in **`TextCoordinateSpace.ScreenPixels`** often pairs with **`ViewportAnchor2D`** so HUD labels track **`ActiveCameraViewportSize`** (the camera's virtual canvas, not the letterboxed physical window).
...
Within each phase, order is still **global registration order**: each entry is **`ISystem`** (serial), **`IParallelSystem`** (parallel), or **`ISingletonSystem`** (serial, single-row), in the order registered. Because registrations come from mods during **`LoadAll`**, the practical order is: engine early mod(s) → gameplay/content mods → engine late mod(s), all governed by `manifest.json` **`loadOrder`** then id. **Early** vs **Fixed** vs **Late** still comes from the interfaces each system implements.

### Rendering (`Rendering/`)

- **`IRenderer`** (implemented by **`VulkanRenderer`**) — mod-facing API: **`SubmitSprite`**, **`SubmitTextGlyph`** / **`SubmitTextGlyphs`** (dedicated text path), **`SubmitPointLight`**, **`SubmitSpotLight`**, **`SubmitDirectionalLight`**, **`SubmitAmbientLight`**, **`SubmitPostProcessVolume`**, **`SetGlobalPostProcess`**, **`SubmitCamera`**, **`RegisterTextureRgba`**, **`RegisterTextureRgbaLinear`** (MSDF atlas pages, linear UNORM sampling), **`TryUploadTextureRgbaSubregion`**, **`WhiteTextureId`**, **`DefaultNormalTextureId`**, **`MissingTextureId`** (magenta/green checkerboard when content fails), **`CreateShaderModuleFromSpirv`**, **`CreateShaderModuleFromGlsl`** (runtime fallback path; logs warning), **`RequestClose`**, plus **`SwapchainPixelSize`** (physical window) and **`ActiveCameraViewportSize`** (camera virtual canvas, used for HUD layout). **CPU-side** submit queues are **thread-safe** for **`IParallelSystem`** workers; GPU command recording and **`DrawFrame`** stay on the render thread.
...
- **HDR frame pipeline (scene-linear offscreen targets, tonemap in composite):** each frame has a **CPU frame plan** phase followed by **GPU recording**. The CPU phase (`FramePlanBuilder.Build`) drains concurrent submission queues, promotes emissive sprites into synthetic point lights, sorts lights deterministically, applies **viewport cull** (`LightViewportCulling` drops off-screen point and spot lights), clamps to per-type caps (`SubmissionClamp`), strips `CastsShadow` on all surviving lights when shadows are globally disabled, and builds a `ShadowSdfCamera` projection snapshot. GPU recording then uploads light SSBOs and runs **CPU tile binning** (`TiledLightCullingCpu` writes per-tile index lists covering the physical viewport), followed by the GPU pass chain: **emissive prepass** (optional per-texel **`EmissiveTextureId`**) → **G-buffer** (opaque sprites only) → **SDF shadow pass** (GPU JFA: occluder mask → JFA init → JFA steps → JFA-to-SDF; **`ShadowDistanceFieldCpu`** exists as a test oracle only) → **tiled deferred lighting** (`tiled_deferred_lighting.frag.glsl`: fullscreen pass evaluates ambient / directional / spot / point lights per tile, cone-tracing the SDF for soft shadows) + emissive bleed → **weighted blended OIT** for **transparent** sprites → resolve to HDR → **bloom** → **composite** to the swapchain. Sort order for sprites is **layer → sort key → depth hint**; **straight alpha** where applicable. See **`.cursor/rules/cyberland-design-goals.mdc`** for linear-color and modularity goals.
- **Dynamic shadows (GPU JFA SDF + cone trace):** during GPU recording the renderer constructs a **signed distance field** on the GPU via Jump Flood Algorithm: occluder mask → JFA seed → iterative JFA steps → distance-to-SDF conversion (`shadow_occluder.{vert,frag}`, `jfa_init.frag`, `jfa_step.frag`, `jfa_to_sdf.frag`). The tiled deferred lighting shader (`tiled_deferred_lighting.frag.glsl`) includes `shadow_sdf_sampling.glsl` and performs an Iñigo Quilez soft-shadow cone trace from each shaded fragment toward each light's **world** position; visibility = `clamp(min(k * d(t) / t), 0, 1)` gives a natural penumbra. The single SDF is shared by every shadow-casting light — no per-light atlas tiles, no per-light shadow draws, soft shadows for free — up to 1024 point lights, 256 spot lights, and 16 directional lights per frame. Bake-time and runtime fragment compilation merge the include into `tiled_deferred_lighting.frag.glsl` (see **`EngineShaderSources.LoadFragmentWithShadowInclude`**). **`ShadowDistanceFieldCpu`** is a CPU test oracle for verifying JFA correctness; it is not used in production rendering. Tune shadow quality via **`ShadowSettings`** in **`GlobalPostProcessSettings`**: `SdfScale`, `ConeTraceSamples`, `SoftShadowK`, `DepthBias`, `DirectionalTraceWorldDist`. **Quality ladder:** on integrated GPUs lower `ConeTraceSamples` to 16 and `SdfScale` to 0.5 (quarter-area SDF); on modern discrete GPUs use the defaults (32 samples, scale 1.0). Per-fragment cost scales with `lightsInTile × ConeTraceSamples`. The tile grid auto-scales tile size at extreme resolutions (`TiledLightCullingCpu.ResolveEffectiveTileGrid`) so all screen regions receive point/spot lighting regardless of window size.
- **Emissive auto-promotion:** any sprite with **`EmissiveIntensity ≥ EmissivePromotionSettings.EmissiveLightThreshold`** is automatically promoted into a synthetic shadow-casting point light at frame build — bright emissive surfaces illuminate and shadow their neighbors with **zero authoring change**. Tune via **`EmissivePromotionSettings`** in **`GlobalPostProcessSettings.EmissivePromotion`**: `EmissiveLightThreshold`, `MaxPromotedLightsPerFrame`, `EmissivePromotionRadiusGain`, `EmissivePromotionIntensityGain`. When global shadows are disabled, `CastsShadow` is stripped on all surviving lights (authored and promoted) so cone-trace loops are skipped entirely. Viewport / HUD sprites are excluded from promotion.
...
- **`ILocalizedContent`** / **`LocalizedContent`** — single façade for **strings** and **localized media paths**. Call **`MergeStringTableAsync("snake.json")`** (table file name only; IO uses the same layered VFS as **`ModLoadContext.VirtualFileSystem`**) so the engine loads **`Content/Locale/en/…`**, then parent cultures, then the active language (later merges override keys). Use **`TryResolveLocalizedPath`**, **`TryLoadLocalizedBytesAsync`**, **`TryLoadTextureFromCanonical`** / **`TryLoadLocalizedTexture`** (returns **`MissingTextureId`** on failure), **`TryLoadLocalizedTextureAsync`**, **`TryResolveLocalizedAtlas`**, or **`TryOpenLocalizedRead`** for player-facing textures, atlas manifests, and audio/video instead of hardcoding **`Locale/en/…`**. Atlas manifests and page PNGs follow the same overlay chain under **`Locale/{culture}/Textures/Atlases/`**.
- **Sprite atlases (`Assets/SpriteAtlasCatalog`)** — gameplay atlases ship as **`Content/Textures/Atlases/*.atlas.json`** plus referenced PNG pages. Scene JSON uses **`cyberland.engine/sprite-atlas-binding`** with a canonical **`manifestPath`** and **`region`** / **`animation`** / **`sheet`** name; the engine resolves locale overlays, uploads one GPU texture per page, and writes **`Sprite.AlbedoTextureId`** + **`UvRect`**. UI JSON **`sourceTexture`** accepts **`manifest.json#region`**, supports **`nineSlice`**, and batches through the same instanced overlay path. Regenerate packed atlases from loose PNG folders with **`.\scripts\Generate-SpriteAtlases.ps1`**. Mod code can call **`ModLoadContext.LoadSpriteAtlas(...)`** or read **`GameHostServices.SpriteAtlasCatalog`**.
...
- **`ModLoadContext.Scenes`** — optional **`ISceneRuntime`** (same instance as **`Host.Scenes`**): **`BeginLoad`** allocates a new additive world; the host pumps **`PumpAsync`** on the render tick—do not **`await`** long scene loads inside **`ModLoader.LoadAll`** (GPU deadlock risk; see **`LoadBakedMsdfAtlasAsync`** docs). File-backed scenes live under each mod’s **`Content/Scenes/*.json`** (see **`RuntimeScenes/Serialization/SceneDocument`**). Retained HUD trees live in **`Content/Ui/*.json`** and attach via scene **`uiPath`** on **`ui-document-root`** (see **`RuntimeUi/`**, **`.cursor/rules/cyberland-ui-json-authoring.mdc`**).
...
- **`GameHostServices`** — **`Renderer`** (**`IRenderer?`**; concrete type **`VulkanRenderer`**), **`Input`** (**`IInputService?`**), **`LocalizedContent`** (**`ILocalizedContent?`**), built-in **`Fonts`** (**`FontLibrary`**) and **`TextGlyphCache`** for **`TextRenderSystem`** / **`TextRenderer`**, optional **`Tilemaps`** (**`ITilemapDataStore?`**) and **`Particles`** (**`ParticleStore?`**) for tile indices and CPU particle buffers used by engine render/sim systems. **`Ui`** (**`IUiRuntime?`**, via **`InitializeRuntimeUi`**) deserializes retained HUD JSON; **`UiDocuments`** maps HUD entities to element ids after attach. **`LastPresentDeltaSeconds`** is wall time between draws (matches ECS **`deltaSeconds`** in the stock host, which runs **`RunFrame`** once per **Render** tick). Populated by **`GameApplication`** after the window and device exist, then passed into **`ModLoadContext`** so mods do not use static globals. **`RuntimeScenes`** (**`SceneRuntime`**) owns additive **`World`** + **`SystemScheduler`** stacks; **`SessionClock`** advances once per frame on the host; **`InGameLoadProgress`** tracks in-game load phases separately from cold-start **`StartupProgress`**.
...
- **Components** — **`Transform`** (local/world 2D homogeneous matrices + optional parent, with lazily decomposed position/rotation/scale properties), **`Sprite`** (includes **`Transparent`** for WBOIT vs deferred opaque path; optional **`EmissiveTextureId`**), **`SpriteLocalizedAsset`** (localized single PNG path binding), **`SpriteAtlasBinding`** (localized atlas manifest + region/animation/sheet), **`BitmapText`** (HUD strings; pair with **`Transform`** for baseline), **`Tilemap`**, **`SpriteAnimation`**, **`ParticleEmitter`**, **`CameraFollow2D`** (fixed-step camera target follow).
...
- **Lighting** — queue **`SubmitPointLight`**, **`SubmitSpotLight`**, **`SubmitDirectionalLight`**, and **`SubmitAmbientLight`** on **`context.Host.Renderer`** each frame you need them (same **`IRenderer`** as sprites). The tiled deferred lighting pass evaluates all submitted light types per screen tile (ambient, directional, spot, point — up to engine caps per type).
...
- **`src/Cyberland.Engine/Rendering/`** (embedded **`Rendering/Shaders/*.glsl`** + precompiled **`Rendering/Shaders/Spirv/*.spv`**) — Vulkan 2D pipeline implementation and shaders; build-time baking runs through **`tools/Cyberland.ShaderBaker`**. Runtime prefers embedded SPIR-V and falls back to GLSL compilation with warnings when precompiled blobs are unavailable/invalid. See **`VulkanRenderer.cs`** (swapchain/present), **`VulkanRenderer.Deferred.Recording.cs`** (per-frame pass order), **`DeferredRenderingConstants.cs`** for HDR/bloom topology, **`tiled_deferred_lighting.frag.glsl`** + **`shadow_sdf_sampling.glsl`** for the tiled lighting and SDF cone-trace shaders, and **`RenderPassDependencyModel.cs`** for the canonical pass DAG.
