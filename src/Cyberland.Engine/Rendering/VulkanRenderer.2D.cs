...
/// <summary>2D HDR: emissive prepass, deferred G-buffer + lighting, WBOIT transparency, bloom, composite (scene-linear).</summary>
public sealed unsafe partial class VulkanRenderer
{
    ...
    private DescriptorManager? _descriptorManager2D;
    private OffscreenTargets? _offscreenTargets2D;
    private PipelineFactory? _pipelineFactory2D;
    private TextureUpload? _textureUpload2D;
    private RenderFrameRecorder? _renderFrameRecorder2D;
    private IFramePlanBuilder? _framePlanBuilder2D;
    private IRenderBackendExecutor? _renderBackendExecutor2D;
    private PostProcessGraph? _postProcessGraph2D;
    ...
    private void Create2DGraphicsPipelineAndSurfaces()
    {
        CreateLinearSampler();
        ...
        CreateSwapchainFramebuffers2D();
        CreateDescriptorLayoutsAndPool();
        _pipelineFactory2D ??= new PipelineFactory(this);
        _pipelineFactory2D.CreateAllPipelines();
    ...
    private void RecreateSwapchainDependent2D()
    {
        Recreate2DOffscreenTargets();
        CreateSwapchainFramebuffers2D();
    }

    private void CreateSwapchainFramebuffers2D()
    {
    ...
    private void CreateDeviceLocalImage2D(uint w, uint h, Format format, ImageUsageFlags usage, out Image img, out DeviceMemory mem, out ImageView view)
    {
        _offscreenTargets2D ??= new OffscreenTargets(this);
        _offscreenTargets2D.CreateDeviceLocalImage2D(w, h, format, usage, out img, out mem, out view);
    }

    private void CreateOffscreenImagesAndFramebuffers()
    {
        _offscreenTargets2D ??= new OffscreenTargets(this);
        _offscreenTargets2D.CreateOffscreenImagesAndFramebuffers();
    }

    private void CreateBloomHalfResTargets()
    {
        _offscreenTargets2D ??= new OffscreenTargets(this);
        _offscreenTargets2D.CreateBloomHalfResTargets();
    }

    private void Recreate2DOffscreenTargets()
    {
        _offscreenTargets2D ??= new OffscreenTargets(this);
        _offscreenTargets2D.Recreate2DOffscreenTargets();
    }

    private void DestroyOffscreenFramebuffer(ref Framebuffer fb, ref ImageView view, ref Image img, ref DeviceMemory mem)
    {
        _offscreenTargets2D ??= new OffscreenTargets(this);
        _offscreenTargets2D.DestroyOffscreenFramebuffer(ref fb, ref view, ref img, ref mem);
    }
    ...
    {
        _descriptorManager2D ??= new DescriptorManager(this);
        _descriptorManager2D.AllocateCompositeDescriptorSet();
    }
    ...
    {
        _descriptorManager2D ??= new DescriptorManager(this);
        _descriptorManager2D.UpdateCompositeBloomSource(bloomFinalView);
    }

    private void AllocateBloomDescriptorSets()
    {
        _descriptorManager2D ??= new DescriptorManager(this);
        _descriptorManager2D.AllocateBloomDescriptorSets();
    }
    ...
    {
        _descriptorManager2D ??= new DescriptorManager(this);
        _descriptorManager2D.UpdateBloomGaussianDescriptorSets();
    }

    private void UpdateBloomExtractDescriptorSet()
    {
        _descriptorManager2D ??= new DescriptorManager(this);
        _descriptorManager2D.UpdateBloomExtractDescriptorSet();
    }

    private void UpdateBloomUpsampleDescriptorSet(ImageView coarseView, ImageView fineView)
    {
        _descriptorManager2D ??= new DescriptorManager(this);
        _descriptorManager2D.UpdateBloomUpsampleDescriptorSet(coarseView, fineView);
    }

    private void AllocateEmissiveSceneDescriptorSet()
    {
        _descriptorManager2D ??= new DescriptorManager(this);
        _descriptorManager2D.AllocateEmissiveSceneDescriptorSet();
    }

    private void UpdateEmissiveSceneDescriptorSet()
    {
        _descriptorManager2D ??= new DescriptorManager(this);
        _descriptorManager2D.UpdateEmissiveSceneDescriptorSet();
    }

    private void AllocateLightingDescriptorSet()
    {
        _descriptorManager2D ??= new DescriptorManager(this);
        _descriptorManager2D.AllocateLightingDescriptorSet();
    }

    private void UpdateLightingDescriptorSet()
    {
        _descriptorManager2D ??= new DescriptorManager(this);
        _descriptorManager2D.UpdateLightingDescriptorSet();
    }
    ...
    private void Destroy2DGraphicsResources()
    {
        if (_vk is null)
            return;

        _pipelineFactory2D ??= new PipelineFactory(this);
        _pipelineFactory2D.DestroyPipelineAndShaderObjects();
        ...
        _offscreenTargets2D ??= new OffscreenTargets(this);
        _offscreenTargets2D.DestroyGbufferAndWboitAndComposite();
    ...
    private void RecordFullFrame2D(CommandBuffer cmd, Framebuffer swapFb)
    {
        _renderFrameRecorder2D ??= new RenderFrameRecorder(this);
        _renderFrameRecorder2D.Record(cmd, swapFb);
    }

    private void RecordFullFrame2DCore(CommandBuffer cmd, Framebuffer swapFb)
    {
        _framePlanBuilder2D ??= new FramePlanBuilder(this);
        _renderBackendExecutor2D ??= new RenderBackendExecutor(this);
        var framePlan = _framePlanBuilder2D.Build();
        _renderBackendExecutor2D.Record(cmd, swapFb, in framePlan);
    }
    ...
    private void ExecuteFramePlanCore(CommandBuffer cmd, Framebuffer swapFb, in FramePlan framePlan)
    {
        ...
        _postProcessGraph2D ??= new PostProcessGraph(this);
        var ppContext = new PostEffectContext(cmd, swapFb, framePlan, vp, sci, vpHalf, sciHalf);
        _postProcessGraph2D.Record(in ppContext, bloomOn, bloomGain, bloomRadius, post);
    ...
    {
        _textureUpload2D ??= new TextureUpload(this);
        return _textureUpload2D.RegisterTextureRgbaInternal(rgba, width, height);
    }

    private void UploadBuffer(ReadOnlySpan<byte> data, ulong size, out VkBuffer buf, out DeviceMemory bmem)
    {
        _textureUpload2D ??= new TextureUpload(this);
        _textureUpload2D.UploadBuffer(data, size, out buf, out bmem);
    }

    private void OneTimeCommands(Action<CommandBuffer> record)
    {
        _textureUpload2D ??= new TextureUpload(this);
        _textureUpload2D.OneTimeCommands(record);
    }

    private void CreateDefaultTextures()
    {
        _textureUpload2D ??= new TextureUpload(this);
        _textureUpload2D.CreateDefaultTextures();
    }
}
